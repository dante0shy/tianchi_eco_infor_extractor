from tianchi_extractor.lstm_extractor.data_loader import Data_loader
import os
import time
...
def evaluate(sess,val_model , data, global_step=None,val_summary = None):
    ...
    t=0
    total_num_x = 0
    metrix= np.zeros([len(label_dict),len(label_dict)]).astype(np.int32)
    num = 0
    for step, (x,d, y, mask_x) in enumerate(data.load_data_batch(batch_size=Config.batch_size)):

        fetches = [val_model.logits,val_model.trans_params,val_model.loss,val_model.out,val_model.viterbi_sequence,val_model.viterbi_score]#
        # fetches = val_model.correct_num
        feed_dict = {}
        feed_dict[val_model.input_data] = x
        feed_dict[val_model.target] = y
        feed_dict[val_model.mask] = mask_x
        feed_dict[val_model.seq_class] = d
        feed_dict[val_model.keep_prob] = 1.
        feed_dict[val_model.is_training] = False
        val_model.assign_new_batch_size(sess, len(x))
        logits,trans_pram,loss,out,seq,score= sess.run(fetches, feed_dict)#
        # seq,score = val_model.predict(logits,mask_x,trans_pram)
        ...
        if not step%100:
            print('heart beat {} : cost {}'.format(step,total_loss))
        num += len(x)
    print(float(c)/t)
...
def run_epoch(sess,model,data_t,data_v,global_step,train_summary,val_summary):
    num = 0
    accuracy_t = 0
    cost_t = 0
    trans_params = None
    for step, (x,d,y,mask) in enumerate(data_t.load_data_batch(batch_size=Config.batch_size)):

        feed_dict = {}
        feed_dict[model.input_data] = x
        feed_dict[model.target] = y
        feed_dict[model.seq_class] = d
        ...
        # print('heart beat {} : cost {}'.format(step,cost))
        cost_t += len(x)*(cost-cost_t)/(len(x)+num)
        if not step % 100:
            print('heart beat {} : cost {}'.format(step, cost_t))
        num+=len(x)
        global_step += 1
    valid_accuracy,val_loss = evaluate(sess, model , data_v, global_step, val_summary)
    ...
    return  global_step,valid_accuracy,trans_params
...
def train_step():
    ...
    loader_train = Data_loader(config.train_data)
    loader_val = Data_loader(config.val_data)
    # loader.load_data()
    with tf.Graph().as_default(),tf.Session(config=tf_config) as sess:
        ...
        checkpoint_dir = os.path.abspath(os.path.join(config.out_dir, "checkpoints_v1_no_high_"))
        checkpoint_prefix = os.path.join(checkpoint_dir, "model")
        if not os.path.exists(checkpoint_dir):
            os.makedirs(checkpoint_dir)
        saver = tf.train.Saver(tf.global_variables())
        ckpt = None
        ckpt = tf.train.get_checkpoint_state("/home/xinmatrix/TwoTB/tianchi/extras/model/checkpoints_v1")
        ...
        for i in range(config.num_epoch):
            ...
            global_steps,val_acc,trans_params=run_epoch(sess, model, loader_train,loader_val, global_steps, train_summary_writer, val_summary_writer)

            if i% config.checkpoint_every==0 or (val_acc>val_acc_max and val_acc>=0.8):
                path = saver.save(sess,checkpoint_prefix,global_steps)
                pkl.dump(trans_params,open(checkpoint_dir+'tras.pkl','w'))
